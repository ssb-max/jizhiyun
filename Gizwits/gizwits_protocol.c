...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwendu)
    {
        info->event[info->num] = EVENT_wendu;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewendu,(uint8_t *)&issuedData[byteFieldOffset],wendu_LEN);
        byteFieldOffset += wendu_LEN;
        dataPoints->valuewendu = gizX2Y(wendu_RATIO,  wendu_ADDITION, elongateP0FormTmp.devStatus.valuewendu); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewendu != cur->valuewendu)
    {
        GIZWITS_LOG("valuewendu Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwendu = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagwendu)
    {
        devStatusTmp.valuewendu = gizY2X(wendu_RATIO,  wendu_ADDITION, dataPoints->valuewendu); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewendu,wendu_LEN);
        byteFieldOffset += wendu_LEN;
    }
